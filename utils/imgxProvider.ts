...
import { parseURL, getQuery, withQuery, parseQuery } from 'ufo';
...
export const operationsGenerator = createOperationsGenerator({
  ...
  joinWith: '&',
  formatter: (key: string, value: unknown) => `${key}=${value}`,
});

export const getImage: ProviderGetImage = (src, { modifiers = {} } = {}) => {
  const {
    public: { imgixSecret, imgixHost },
  } = useRuntimeConfig();
  const parsed = parseURL(src);
  const query = getQuery(parsed.search);

  const operations = operationsGenerator(modifiers);
  const newQuery = { ...query, ...parseQuery(operations) };

  const signatureBase = imgixSecret + withQuery(parsed.pathname, newQuery);
  const signature = Md5.hashStr(signatureBase);

  newQuery.s = signature;

  const url = 'https://' + imgixHost + withQuery(parsed.pathname, newQuery);
